...
$(BINDIR)/test: vendor
	...
	@env GOPATH="/opt/proj/go:${GOPATH}" go build -race -o $@ kubeds/main.go

$(BINDIR)/test-linux: vendor
	@echo "--> building Linux test binary"
	@env GOOS=linux GOARCH=amd64 GOPATH="/opt/proj/go:${GOPATH}" go build -race -o $@ kubeds/main.go

docker: $(BINDIR)/test-linux
	@echo "--> building test docker image"
	@docker build -t test .

integration: integration.xds
